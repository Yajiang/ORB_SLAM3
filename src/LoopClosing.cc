    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bFindedRegion && mbActiveLC)
            {
                if(mbMergeDetected)
                {
                    if ((mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_STEREO || mpTracker->mSensor==System::IMU_RGBD || mpTracker->mSensor==System::IMU_CAMERA_RIG) &&
